...
{images && images.length > 0 && (
  <div className="w-full flex flex-col items-center justify-center flex-1">
    {/* --- MOBILE: carousel à un item par vue, snap */}
    <div
      ref={mobileScrollRef}
      onScroll={handleMobileScroll}
      className="w-full sm:hidden overflow-x-auto snap-x snap-mandatory touch-pan-x scrollbar-thin"
      ...
      <div className="flex w-full">
        {images.map((img, i) => (
          <div
            ...
            // assure que chaque slide prend toute la largeur visible
            style={{ width: "100%" }}
          >
            <img
              src={img}
    ...
    <div className="flex gap-2 mt-2 sm:hidden">
      {images.map((_, i) => (
        <button
          ...
          className={`w-2 h-2 rounded-full transition-transform ${i === activeIndex ? "bg-yellow-400 scale-110" : "bg-gray-300"}`}
      ...
      <div className="flex gap-2 justify-center mt-4">
        {images.map((_, i) => (
          <button
            key={i}
            onClick={() => setActiveIndex(i)}
            className={`w-2 h-2 rounded-full ${i === activeIndex ? "bg-yellow-400" : "bg-gray-300"}`}
